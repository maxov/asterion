  ...
  useFrame(() => {
    ...
    if (Math.abs(current - targetRef.current) > 0.001) {
      camera.position.setLength(
        MathUtils.lerp(current, targetRef.current, ZOOM_DAMPING),
      );
    }
  });
...
export function Scene() {
  ...
  const saturnLocalSunDirection = useMemo(
    () =>
      sunDirection
        .clone()
        .applyAxisAngle(new Vector3(0, 0, 1), -AXIAL_TILT_RAD)
        .normalize(),
    [sunDirection],
  );
  ...
  return (
    <>
      ...
      <group rotation={[0, 0, AXIAL_TILT_RAD]}>
        <Saturn
          localSunDirection={saturnLocalSunDirection}
      ...
      <Stars />
      <Lighting direction={sunDirection} intensity={sunIntensity} />
      <Effects />
    </>
  );
}
