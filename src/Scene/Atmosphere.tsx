...
export function Atmosphere() {
  ...
  const { intensity, power } = useControls('Atmosphere', {
    intensity: { value: 0.15, min: 0, max: 1, step: 0.01, label: 'Glow Intensity' },
  ...
  useEffect(() => { intensityUniform.value = intensity }, [intensityUniform, intensity])
  useEffect(() => { powerUniform.value = power }, [powerUniform, power])
